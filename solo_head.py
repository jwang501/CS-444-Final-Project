...
class SOLOHead(pl.LightningModule):
    _default_cfg = {
        ...
        'postprocess_cfg': dict(cate_thresh=0.4, mask_thresh=0.5, pre_NMS_num=50, keep_instance=5, IoU_thresh=0.5)
    }
    ...
    def __init__(self, **kwargs):
        ...
        pretrained_model = torchvision.models.detection.maskrcnn_resnet50_fpn(weights='DEFAULT', weights_backbone='DEFAULT')
    ...
    def forward_single_level(self, fpn_feat, idx, eval=False, upsample_shape=None):
        ...
        if not eval:
            ...
            assert category_prediction.shape[1:] == (1, current_level_grid, current_level_grid), "cat prediction shape is not right"
    ...
    def category_loss(self, pred, target):
        ...
        target_onehot = F.one_hot(torch.stack(target).to(torch.int64), num_classes=2)
    ...
    def configure_optimizers(self):
        ...
        scheduler = {
            'scheduler': MultiStepLR(optimizer, milestones=[26, 32], gamma=0.1),
        }
